...
def events(request):
"""
GET /api/events/?sports=basketball_nba,baseball_mlb,soccer_epl[&debug=1]
Returns normalized events. With debug=1, includes per-sport upstream status.
"""
...
all_events = []
debug_info = {"has_key": bool(api_key), "sports": []}

if not api_key:
# Early return if key missing; normal response stays an empty list,
# but with debug=1 you’ll see why.
if want_debug:
return JsonResponse({"events": [], "debug": debug_info})
...
status = -1
if want_debug:
debug_info["sports"].append({"sport": sport, "status": status, "error": str(e)})
continue

if want_debug:
# Don’t include full body; just a small hint
quota = r.headers.get("x-requests-remaining") or r.headers.get("X-Requests-Remaining")
debug_info["sports"].append({"sport": sport, "status": status, "quota_left": quota})

if status != 200:
# Silently skip in normal mode; noted in debug.
continue
...
if want_debug:
return JsonResponse({"events": all_events, "debug": debug_info})
return JsonResponse(all_events, safe=False)
